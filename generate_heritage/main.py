...
raw_mentor = {
    ...
    "2016": ["Daniel Alabi", "Kevin Yeh", "Kimberly Leon", "Manny Lopez", "Emily Anders (Pries)", "Simon Ayzman"],
    "2017": ["Claire Durand", "Dan Gorelick", "Sakib Jalal", "Sruti Modekurty", "Su Wang", "Emily Anders (Pries)", "Simon Ayzman"],
...
for mentor_year, mentor_arr in raw_mentor.items():
    ...
    for person in mentor_arr:
        ...
        if len([x for x in fellows if x[0] == fname and x[1] == lname]):
            ...
            year = [x[2] for x in fellows if fname in x[3] or lname in x[3]][0]
            # print("match:", person, year)
            mentor[mentor_year].append((person, year))
